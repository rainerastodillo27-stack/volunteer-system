...
# 1. Push any local commits to GitHub
Write-Host "🚀 [1/3] Pushing commits to GitHub..." -ForegroundColor Yellow
git push origin main
if ($LASTEXITCODE -ne 0) {
    Write-Host "  ⚠ Push check finished. Continuing..." -ForegroundColor Gray
} else {
    Write-Host "  ✓ Git branch is up-to-date with GitHub." -ForegroundColor Green
}

# 2. Trigger pull and restart on VPS
Write-Host ""
Write-Host "📡 [2/3] Updating VPS at $Server..." -ForegroundColor Yellow
