...
def main() -> None:
    ...
    migrated_report_tables = maintenance_summary.get("migrated_report_tables", [])
    ...
    if migrated_report_tables:
        print(f"Migrated legacy report rows into reports: {', '.join(migrated_report_tables)}")
    if archived_records:
