...
DATA_QUALITY_CONSTRAINT_SPECS = [
    ...
    ("users", "users_id_len_chk", "length(users_id) between 1 and 64"),
    ...
    ("volunteers", "volunteers_id_len_chk", "length(volunteers_id) between 1 and 64"),
    ...
    ("programs", "programs_category_chk", "category is null or category in ('Education', 'Livelihood', 'Nutrition', 'Disaster')"),
    ("programs", "programs_program_module_chk", "program_module is null or program_module in ('Education', 'Livelihood', 'Nutrition', 'Disaster')"),
    ("programs", "programs_title_len_chk", "length(title) between 1 and 150"),
    ("programs", "programs_description_len_chk", "description is null or length(description) <= 3000"),
    ("status_updates", "status_updates_id_len_chk", "length(status_updates_id) between 1 and 64"),
    ...
    ("volunteer_matches", "volunteer_matches_id_len_chk", "length(volunteer_matches_id) between 1 and 64"),
    ...
    ("volunteer_time_logs", "volunteer_time_logs_id_len_chk", "length(volunteer_time_logs_id) between 1 and 64"),
    ("volunteer_time_logs", "volunteer_time_logs_note_len_chk", "note is null or length(note) <= 1000"),
    ("volunteer_time_logs", "volunteer_time_logs_attendance_photo_len_chk", "attendance_photo is null or length(attendance_photo) <= 400000"),
    ("volunteer_time_logs", "volunteer_time_logs_completion_photo_len_chk", "completion_photo is null or length(completion_photo) <= 400000"),
    ("volunteer_time_logs", "volunteer_time_logs_completion_report_len_chk", "completion_report is null or length(completion_report) <= 10000"),
    ("volunteer_time_logs", "volunteer_time_logs_attendance_checked_by_len_chk", "attendance_checked_by is null or length(attendance_checked_by) <= 64"),
    ("volunteer_time_logs", "volunteer_time_logs_attendance_checked_by_name_len_chk", "attendance_checked_by_name is null or length(attendance_checked_by_name) <= 120"),
    ("volunteer_event_joins", "volunteer_event_joins_id_len_chk", "length(volunteer_event_joins_id) between 1 and 64"),
    ...
    ("reports", "reports_id_len_chk", "length(reports_id) between 1 and 64"),
...
def prune_stale_legacy_app_users(connection: Any) -> list[str]:
    with connection.cursor() as cursor:
        ...
        reference_guards = ["not exists (select 1 from users u where u.users_id = a.app_users_id)"]
        if messages_exists:
