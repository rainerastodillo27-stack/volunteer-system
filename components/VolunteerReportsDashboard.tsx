...
export function PartnerReportsDashboard({
  ...
  const quarterlyReportContent = useMemo(() => {
    if (activeSummary) {
      return buildProjectSummaryContent(activeSummary);
    }

    return [
      `Quarter: ${currentQuarter.label}`,
      `Reporting Period: ${currentQuarter.periodLabel}`,
      `Organization: ${orgName}`,
      `Program: ${programTitle}`,
      `Status: ${reportStatus}`,
      `Submitted On: ${submittedOn}`,
      '',
      activeReport?.description || 'No report details were submitted for this quarter.',
    ].join('\n');
  }, [activeReport, activeSummary, currentQuarter, orgName, programTitle, reportStatus, submittedOn]);

  const handleDownloadDocument = (document: PartnerQuarterlyDocument) => {
    if (document.url) {
      Linking.openURL(document.url).catch(() => Alert.alert('Unable to open document'));
      return;
    }

    // Some summary documents are generated by this screen and do not have a
    // stored file yet. Give every download control a real downloadable result.
    void downloadPdfFile(
      `${document.id}-${currentQuarter.label.replace(/\s+/g, '-')}.pdf`,
      buildTextPdf(document.title, quarterlyReportContent)
    );
  };
